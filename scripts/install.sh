...
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/ESRAILHAQUE/gitswitch/master/scripts/install.sh | bash
#   curl -fsSL ... | bash -s -- --version v1.0.3
...
info()  { printf '\033[0;32m[gitswitch]\033[0m %s\n' "$*"; }
warn()  { printf '\033[0;33m[gitswitch]\033[0m %s\n' "$*" >&2; }
error() { printf '\033[0;31m[gitswitch] error:\033[0m %s\n' "$*" >&2; exit 1; }
...
install_via_release() {
  ...
  info "Installing gitswitch ${TAG} (${OS}/${ARCH}) to ${INSTALL_DIR}..."
...
if command -v gitswitch >/dev/null 2>&1; then
  info "Installed: $(command -v gitswitch)"
  gitswitch --version
else
  warn "gitswitch was installed to ${INSTALL_DIR}/${BINARY} but is not on your PATH yet"
  export PATH="${INSTALL_DIR}:$PATH"
fi

path_hint

printf "\n\033[0;32m[gitswitch]\033[0m Let's set up your GitHub profiles!\n"
printf "How many GitHub accounts do you want to set up right now? (Enter 0 to skip) [0]: "
read -r num_accounts < /dev/tty || num_accounts=0
...
if [[ "$num_accounts" =~ ^[1-9][0-9]*$ ]]; then
  for ((i=1; i<=num_accounts; i++)); do
    printf "\n\033[0;36m--- Setting up Account %d of %d ---\033[0m\n" "$i" "$num_accounts"
    gitswitch gen < /dev/tty || true
  done
  printf "\n\033[0;32m[gitswitch]\033[0m Setup complete! Make sure you have copied the SSH keys and added them to your GitHub settings.\n"
else
  info "Skipping setup. Run 'gitswitch gen' anytime to create a profile."
fi

printf "\n\033[0;36m--- Configuring Auto-Alias ---\033[0m\n"
case "$(basename "${SHELL:-}")" in
  fish) 
    mkdir -p ~/.config/fish
    if ! grep -q 'alias git="gitswitch"' ~/.config/fish/config.fish 2>/dev/null; then
      echo 'alias git="gitswitch"' >> ~/.config/fish/config.fish
    fi
    ;;
  zsh)  
    if ! grep -q 'alias git="gitswitch"' ~/.zshrc 2>/dev/null; then
      echo 'alias git="gitswitch"' >> ~/.zshrc
    fi
    ;;
  *)    
    if ! grep -q 'alias git="gitswitch"' ~/.bashrc 2>/dev/null; then
      echo 'alias git="gitswitch"' >> ~/.bashrc
    fi
    ;;
esac
printf "\033[0;32m[gitswitch]\033[0m Successfully aliased 'git' to 'gitswitch'.\n"
printf "Please restart your terminal or run \033[0;33msource ~/.bashrc\033[0m (or equivalent) to apply changes.\n\n"
